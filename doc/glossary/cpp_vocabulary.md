...
EBO is a compiler optimization technique. In C++, even an empty struct (one with no data members) 
...
## Aggregate type
An Aggregate is a specific kind of class or struct in C++ that is "simple" enough to be initialized 
...
Introduced in C++20, a Structural Type is a class type that can be used as a Non-Type Template Parameter 
(NTTP).
...
The Tuple Protocol is a set of standardized "hooks" in the C++ Standard Library. By implementing these 
...
Provide type.get<I>() or get<I>(type); //A function to access the element.
```
Why is this useful?
...
In C++ containers are structures built in order to store data. There can be different
