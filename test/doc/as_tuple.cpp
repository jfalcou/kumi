...
template<std::size_t I> decltype(auto) get(vec3 const& v) noexcept
{
...
template<std::size_t I> decltype(auto) get(vec3& v) noexcept
{
  if constexpr (I == 0) return v.x;
  if constexpr (I == 1) return v.y;
  if constexpr (I == 2) return v.z;
...
template<> struct kumi::is_product_type<vec3> : std::true_type
{
};

// Adapt as structured bindable type
template<> struct std::tuple_size<vec3> : std::integral_constant<std::size_t, 3>
{
};

template<std::size_t I> struct std::tuple_element<I, vec3>
{
  using type = float;
};
