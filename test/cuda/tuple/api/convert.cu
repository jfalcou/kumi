...
namespace
{
  struct Int
  {
    ...
    __host__ __device__ Int(int v) : value(v) {}
  };
  ...
  __global__ void convert(kumi::tuple<int, int>* out)
  {
    ...
    auto to = static_cast<kumi::tuple<Int, Int>>(in);
...
#if !defined(__NVCC__)
  __global__ void convert_explicit(kumi::tuple<int, int>* out)
  {
    auto in = kumi::tuple{1, 2};
...
#if !defined(__NVCC__)
TTS_CASE("Check tuple to explicitly constructible type device conversion")
{
  kumi::tuple<int, int> out;

  TTS_EXPECT(run_on_device(convert_explicit, out));
  TTS_EQUAL(out, (kumi::tuple{1, 2}));
};
#endif
