...
{
  TTS_TYPE_IS ( (kumi::result::filter_not_t<std::is_integral,kumi::tuple<>>)
              , (kumi::tuple<>)
              );
...
{
  TTS_EQUAL ( (kumi::filter_not<std::is_pointer>(kumi::tuple{}))
            , (kumi::tuple<>{})
            );
};
