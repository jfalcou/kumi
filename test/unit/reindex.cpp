...
TTS_CASE("Check result::reindex<Tuple,Indexes> behavior")
{
  ...
  constexpr auto id1 = kumi::make_index_map(0,1,2,3);
  constexpr auto id2 = kumi::make_index_map(1,2,3,0);
  constexpr auto id3 = kumi::make_index_map(3,3);
  constexpr auto id4 = kumi::make_index_map(1,2,3, kumi::make_index_map(0,1,2));
  ...
  TTS_TYPE_IS( (kumi::result::reindex_t<nested_t, kumi::make_index_map(0)>), (kumi::tuple<kumi::tuple<int, float>>));
};
...
{
  {
    ...
    {
      constexpr auto id = kumi::make_index_map(0,1,2,3);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, t);
    }

    {
      constexpr auto id = kumi::make_index_map(3,2,1,0);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{'5',3.4f,2.,1}));
    }

    {
      constexpr auto id = kumi::make_index_map(0,1,2);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{1,2.,3.4f}) );
    }

    {
      constexpr auto id = kumi::make_index_map(0,1);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{1,2.}) );
    }

    {
      constexpr auto id = kumi::make_index_map(0);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{1}) );
    }

    {
      constexpr auto id = kumi::make_index_map(1);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{2.}) );
    }

    {
      constexpr auto id = kumi::make_index_map(2);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{3.4f}) );
    }

    {
      constexpr auto id = kumi::make_index_map(3);
      auto s = kumi::reindex<id>(t);
      TTS_EQUAL(s, (kumi::tuple{'5'}) );
    }

    {
      constexpr auto id = kumi::make_index_map(0,1,2,3,2,1,0);
      auto s = kumi::reindex<id>(t);
    ...
    {
      constexpr auto id = kumi::make_index_map(0,1,2,3, kumi::make_index_map(3,2,1,0));
      auto s = kumi::reindex<id>(t);
