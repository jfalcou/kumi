...
TTS_CASE("Check kumi::result::chunks_t behavior on records")
{
...
TTS_CASE("Check runtime kumi::chunks behavior on records")
{
  using namespace kumi;
  ...
  auto r3 = kumi::chunks<2>(kumi::to_ref(r_test));
  kumi::for_each(
    [&](auto& tile) {
      auto [f, s] = values_of(tile);
      f += 1;
      s += 2;
      return tile;
    },
  ...
  TTS_EQUAL((chunks<1>(r1)), (make_tuple(record{"a"_id = 1}, record{"b"_id = 3.f}, record{"c"_id = 5.},
                                         record{"d"_id = 'x'}, record{"e"_id = short{77}})));

  TTS_EQUAL((chunks<2>(r2)),
            (make_tuple(record{"a"_id = 112, "b"_id = -18.f},
                        record{"c"_id = record{"d"_id = '1', "e"_id = long{22}, "f"_id = str{"Yes"}}})));

  TTS_EQUAL((chunks<2>(std::move(r2))),
            (make_tuple(record{"a"_id = 112, "b"_id = -18.f},
                        record{"c"_id = record{"d"_id = '1', "e"_id = long{22}, "f"_id = str{"Yes"}}})));

  auto r4 = record{"a"_id = 1, "b"_id = 2, "c"_id = moveonly{}, "d"_id = 'x', "e"_id = 5.f};
  TTS_EXPECT_COMPILES(r4, { kumi::chunks<2>(std::move(r4)); });
};
