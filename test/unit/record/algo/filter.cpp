...
TTS_CASE("Check filter() behavior with values")
{
  ...
  TTS_EQUAL(kumi::filter<std::is_pointer>(original), (kumi::record{"b"_f = &a, "d"_f = &b, "f"_f = &c}));

  TTS_EQUAL(kumi::filter<std::is_floating_point>(original), (kumi::record{"c"_f = b, "e"_f = c}));

  TTS_EQUAL(kumi::filter<std::is_null_pointer>(original), (kumi::record{"h"_f = nullptr}));
...
TTS_CASE("Check filter() behavior with reference")
{
  ...
  TTS_EQUAL(kumi::filter<std::is_lvalue_reference>(original),
            (kumi::record<kumi::field_capture<"b", int&>, kumi::field_capture<"d", double&>>{a, b}));
};
