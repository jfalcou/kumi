...
TTS_CASE("Check result::partition<Func,record> behavior")
{
  ...
  TTS_TYPE_IS(
    (kumi::result::partition_t<
      std::is_integral,
      kumi::record<kumi::field_capture<"a", int>, kumi::field_capture<"b", float>, kumi::field_capture<"c", double>>>),
    (kumi::tuple<kumi::record<kumi::field_capture<"a", int>>,
  ...
  TTS_TYPE_IS((kumi::result::partition_t<std::is_rvalue_reference,
                                         kumi::record<kumi::field_capture<"a", int&>, kumi::field_capture<"b", float&&>,
                                                      kumi::field_capture<"c", double const&>>>),
              (kumi::tuple<kumi::record<kumi::field_capture<"b", float&&>>,
                           kumi::record<kumi::field_capture<"a", int&>, kumi::field_capture<"c", double const&>>>));
};
...
TTS_CASE("Check partition() behavior with values")
{
  ...
  TTS_EQUAL(kumi::partition<std::is_pointer>(original),
            (kumi::tuple{kumi::record{"b"_f = &a, "d"_f = &b, "f"_f = &c},
                         kumi::record{"a"_f = a, "c"_f = b, "e"_f = c, "g"_f = 'z', "h"_f = nullptr}}));

  TTS_EQUAL(kumi::partition<std::is_floating_point>(original),
            (kumi::tuple{kumi::record{"c"_f = b, "e"_f = c},
                         kumi::record{"a"_f = a, "b"_f = &a, "d"_f = &b, "f"_f = &c, "g"_f = 'z', "h"_f = nullptr}}));

  TTS_EQUAL(kumi::partition<std::is_null_pointer>(original), (kumi::tuple{kumi::record{"h"_f = nullptr}, kumi::record{
                                                                                                           "a"_f = a,
                                                                                                           "b"_f = &a,
                                                                                                           "c"_f = b,
                                                                                                           "d"_f = &b,
...
TTS_CASE("Check partition() behavior with reference")
{
  ...
  auto original = kumi::record<kumi::field_capture<"a", int>, kumi::field_capture<"b", int&>,
                               kumi::field_capture<"c", double>, kumi::field_capture<"d", double&>>{a, a, b, b};

  TTS_EQUAL(kumi::partition<std::is_lvalue_reference>(original),
            (kumi::tuple{kumi::record<kumi::field_capture<"b", int&>, kumi::field_capture<"d", double&>>{a, b},
