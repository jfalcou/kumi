...
TTS_CASE("Check result::max/max_flat<...> behavior")
{
  ...
  TTS_TYPE_IS((kumi::result::max_t<kumi::tuple<char, short, double, int>>), double);

  TTS_TYPE_IS((kumi::result::max_t<kumi::tuple<char, short, int, double>, func_t>), std::size_t);

  TTS_TYPE_IS((kumi::result::max_flat_t<kumi::tuple<char, short, int, double>, func_t>), std::size_t);

  TTS_TYPE_IS((kumi::result::max_flat_t<kumi::tuple<char, kumi::tuple<short, int>, double>, func_t>), std::size_t);
};
...
TTS_CASE("Check tuple::max/max_flat behavior")
{
  ...
  auto fcp = kumi::tuple{'e', 2., kumi::tuple{1., short{55}, 'u'}, 3.f, 'z'};
  ...
  TTS_EQUAL((kumi::max(std::move(f0), [](auto m) { return sizeof(m); })), 2 * sizeof(double));
  TTS_EQUAL((kumi::max_flat(std::move(fcp), [](auto m) { return sizeof(m); })), sizeof(double));
