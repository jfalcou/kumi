...
Thank you for your interest in KUMI. Bug reports, documentation fixes, a missing algorithm and whole
new parts of the tuple protocol are all welcome.
...
ask on [our Discord](https://discord.com/channels/692734675726237696/916823794230886481). No
...
merged pull requests. **You are welcome.** A student who wants to see what a compile-time tuple
library looks like from the inside is exactly who we wrote this for, and nobody is going to hold a
...
[Discord](https://discord.com/channels/692734675726237696/916823794230886481): "I have two weeks and
...
issue](https://github.com/jfalcou/kumi/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.tuple.algo.cat.exe
ctest --preset gcc
```

One target per test file, named after its path, and CMake globs the test directories, so a new
`.cpp` needs no CMake edit. Tuples and records are two families with their own trees: what applies
to both is expected on both sides.
...
- **the documentation of what you touched**, with the `kumi-doxygen` target still silent, since a
...
and the types involved. A Godbolt link that reproduces it replaces all three.

**Work on an open issue.** The [open issues](https://github.com/jfalcou/kumi/issues) are all fair
...
[Discord](https://discord.com/channels/692734675726237696/916823794230886481) before you write it: a
...
cmake --build build/gcc --target kumi-doxygen
```
